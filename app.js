(function () {
  ...
  function loadCatalogs() {
    Promise.all([
      fetch('data/foods.json').then(function (res) { return res.ok ? res.json() : []; }).catch(function () { return []; }),
      fetch('data/exercises.json').then(function (res) { return res.ok ? res.json() : []; }).catch(function () { return []; })
    ]).then(function (data) {
  ...
  function renderDashboard() {
    ...
    if (recent) {
      ...
      recent.innerHTML = last.length ? last.map(function (item) {
        return '<div class="recent-row"><div class="food-emoji">◍</div><div class="recent-row-copy"><strong>' + esc(item.name) + '</strong><small>' + esc(item.meal) + ' · ' + format(item.grams) + ' g</small></div><div class="recent-kcal">' + format(item.kcal) + ' kcal</div></div>';
  ...
  function renderExerciseBrowser() {
    ...
    host.innerHTML = list.length ? list.map(function (ex) {
      return '<div class="exercise-row"><span class="exercise-glyph">↗</span><div class="exercise-row-copy"><strong>' + esc(ex.name) + '</strong><small>' + esc(ex.muscle || 'General') + ' · ' + esc(ex.equipment || 'Libre') + '</small></div><button class="add-exercise" data-add-to-routine="' + esc(ex.id) + '" title="Añadir a una rutina">＋</button></div>';
  ...
  function renderActiveWorkout() {
    ...
    host.innerHTML = '<article class="session-card"><div class="session-head"><div><div class="eyebrow">SESIÓN EN CURSO</div><h2>' + esc(active.name) + '</h2></div><div class="timer-display" id="timer-display">00:00</div></div><div class="session-exercises">' + active.exercises.map(function (ex, exIndex) {
      return '<div class="session-exercise"><div class="session-exercise-head"><div><strong>' + esc(ex.name) + '</strong><small>' + esc(ex.muscle || 'General') + '</small></div><small>' + ex.sets.length + ' series</small></div><div class="set-table"><span>Serie</span><span>Reps</span><span>RIR</span><span>kg</span><span>Hecha</span>' + ex.sets.map(function (set, setIndex) {
        return '<span>' + (setIndex + 1) + '</span><input type="number" min="1" max="100" value="' + esc(set.reps) + '" data-session-value="' + exIndex + ',' + setIndex + ',reps" aria-label="Repeticiones"><input type="number" min="0" max="10" value="' + esc(set.rir) + '" data-session-value="' + exIndex + ',' + setIndex + ',rir" aria-label="RIR"><input type="number" min="0" step="0.5" value="' + esc(set.weight || 0) + '" data-session-value="' + exIndex + ',' + setIndex + ',weight" aria-label="Peso en kg"><input type="checkbox" ' + (set.done ? 'checked' : '') + ' data-session-value="' + exIndex + ',' + setIndex + ',done" aria-label="Marcar serie completada">';
      }).join('') + '</div></div>';
  ...
  function foodModalHtml() {
    var tabLabel = foodDraft.tab === 'catalog' ? 'Catálogo' : 'Mi alimento';
    ...
    var customContent = '<form id="custom-food-form"><div class="field"><label for="custom-food-name">Nombre del alimento</label><input id="custom-food-name" name="name" placeholder="Ej. Mi mezcla de frutos secos" required></div><div class="form-grid"><div class="field"><label for="custom-food-kcal">Calorías por 100 g</label><input id="custom-food-kcal" name="kcal" type="number" min="0" step="0.1" required></div><div class="field"><label for="custom-food-protein">Proteína (g)</label><input id="custom-food-protein" name="protein" type="number" min="0" step="0.1" required></div><div class="field"><label for="custom-food-carbs">Carbohidratos (g)</label><input id="custom-food-carbs" name="carbs" type="number" min="0" step="0.1" required></div><div class="field"><label for="custom-food-fat">Grasas (g)</label><input id="custom-food-fat" name="fat" type="number" min="0" step="0.1" required></div></div><button type="submit" class="button button-light full-width">Guardar mi alimento</button></form>';
    return '<div class="modal-header"><div><div class="eyebrow">DIARIO DE HOY</div><h2>Añadir alimento</h2></div><button class="close-modal" data-close-modal>×</button></div><div class="catalog-tabs"><button type="button" data-food-tab="catalog" class="' + (foodDraft.tab === 'catalog' ? 'active' : '') + '">Catálogo (' + foods.length.toLocaleString('es-ES') + ')</button><button type="button" data-food-tab="custom" class="' + (foodDraft.tab === 'custom' ? 'active' : '') + '">Mi alimento</button></div><div class="food-tab-content">' + (foodDraft.tab === 'catalog' ? catalogContent : customContent) + '</div>' + (foodDraft.tab === 'catalog' ? '<div id="selected-food-area">' + (selected ? '<div class="selected-food"><span class="food-emoji">◍</span><div><strong>' + esc(selected.name) + '</strong><small>' + format(selected.kcal) + ' kcal · P ' + format(selected.protein) + ' · C ' + format(selected.carbs) + ' · G ' + format(selected.fat) + ' por 100 g' + (selected.url ? ' · <a class="food-source-link" href="' + esc(selected.url) + '" target="_blank" rel="noopener noreferrer">Ver fuente</a>' : '') + '</small></div></div><div class="form-grid"><div class="field"><label for="food-grams">Cantidad (g)</label><input id="food-grams" type="number" min="1" step="1" value="100" required></div><div class="field"><label for="food-meal">Comida</label><select id="food-meal">' + MEALS.map(function (m) { return '<option ' + (foodDraft.meal === m.id ? 'selected' : '') + '>' + m.id + '</option>'; }).join('') + '</select></div></div><div class="modal-error" id="food-error"></div><div class="modal-footer"><button type="button" class="button button-quiet" data-close-modal>Cancelar</button><button type="button" class="button button-dark" data-confirm-food>Añadir al diario</button></div>' : '<div class="modal-hint" style="margin-top:12px">Selecciona un alimento y después indica la cantidad.</div>') : '</div>';
  }
  ...
  function addFoodToLog() {
    ...
    var grams = num(document.getElementById('food-grams') && document.getElementById('food-grams').value);
    ...
    if (grams <= 0) { if (error) error.textContent = 'Introduce una cantidad mayor que cero.'; return; }
    var ratio = grams / 100;
    state.log.push({
      id: id(), foodId: food.id, name: food.name, brand: food.brand, source: food.source || 'Open Food Facts',
      grams: grams, kcal: round(food.kcal * ratio, 1), protein: round(food.protein * ratio, 1),
  ...
  function saveCustomFood(form) {
    ...
    var item = {
      ...
      kcal: num(values.get('kcal')), protein: num(values.get('protein')), carbs: num(values.get('carbs')), fat: num(values.get('fat')), source: 'Alimento personalizado'
    };
    if (!item.name) return;
    state.customFoods.unshift(item); saveState();
    foodDraft.selected = item.id; foodDraft.tab = 'catalog'; foodDraft.query = item.name;
    modal(foodModalHtml(), true); bindFoodSearch(); toast('Alimento guardado en tu catálogo.');
  }
  ...
  function startWorkout(routineId) {
    ...
    state.activeWorkout = {
      ...
      exercises: routine.exercises.map(function (exerciseId) {
        ...
        return { exerciseId: exerciseId, name: ex.name, muscle: ex.muscle || 'General', sets: [{ reps: 10, rir: 2, weight: 0, done: false }, { reps: 10, rir: 2, weight: 0, done: false }, { reps: 10, rir: 2, weight: 0, done: false }] };
      })
    };
  ...
  function chatContext() {
    ...
    return {
      goals: state.goals,
      today: { consumed: t, remaining: { calories: Math.max(0, state.goals.calories - t.kcal), protein: Math.max(0, state.goals.protein - t.protein), carbs: Math.max(0, state.goals.carbs - t.carbs), fat: Math.max(0, state.goals.fat - t.fat) }, foods: todayLog().map(function (f) { return { name: f.name, grams: f.grams, meal: f.meal, kcal: f.kcal, protein: f.protein, carbs: f.carbs, fat: f.fat }; }) },
