(function () {
  ...
  function openBackupModal() {
    modal('<div class="modal-header"><div><div class="eyebrow">TUS DATOS · SOLO EN TU NAVEGADOR</div><h2>Copiar o transferir datos</h2></div><button class="close-modal" data-close-modal aria-label="Cerrar">×</button></div>' +
      '<p class="backup-description">Descarga una copia para guardarla o pasarla a otro dispositivo. En el otro dispositivo, elige ese archivo para restaurar tus objetivos, comidas, alimentos propios, rutinas y entrenamientos.</p>' +
      '<button type="button" class="button button-dark full-width" data-download-backup>Descargar copia de seguridad (.json)</button>' +
      '<div class="backup-divider"><span>PARA RESTAURAR EN ESTE DISPOSITIVO</span></div>' +
      '<label class="backup-file-control" for="backup-file"><span>Elegir archivo de copia</span><input id="backup-file" type="file" accept="application/json,.json"></label>' +
      '<div class="modal-error backup-error" id="backup-error" role="status"></div>' +
      '<p class="backup-warning">Al restaurar, los datos guardados ahora en este navegador se reemplazarán. FitAI te pedirá confirmarlo antes.</p>');
  }
  function downloadBackup() {
    var backup = { format: 'fitai-data-backup', version: 1, exportedAt: new Date().toISOString(), state: state };
    var blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' });
    var url = URL.createObjectURL(blob);
    var link = document.createElement('a');
    link.href = url; link.download = 'fitai-copia-' + dateKey() + '.json';
    document.body.appendChild(link); link.click(); link.remove();
    window.setTimeout(function () { URL.revokeObjectURL(url); }, 1000);
    toast('Copia descargada. Guárdala para restaurarla en otro dispositivo.');
  }
  function validateBackup(data) {
    if (!data || data.format !== 'fitai-data-backup' || data.version !== 1) throw new Error('Este archivo no es una copia compatible de FitAI.');
    var saved = data.state;
    if (!saved || typeof saved !== 'object' || Array.isArray(saved) || !saved.goals || typeof saved.goals !== 'object') throw new Error('La copia no contiene un estado válido.');
    ['log', 'routines', 'workouts', 'customFoods', 'customExercises', 'chat'].forEach(function (key) {
      if (!Array.isArray(saved[key])) throw new Error('La copia tiene una sección dañada: ' + key + '.');
    });
    if (saved.activeWorkout !== null && saved.activeWorkout !== undefined && (typeof saved.activeWorkout !== 'object' || Array.isArray(saved.activeWorkout))) throw new Error('La sesión activa de la copia no es válida.');
    var goals = Object.assign({}, emptyState.goals, saved.goals);
    ['calories', 'protein', 'carbs', 'fat'].forEach(function (key) {
      var value = Number(goals[key]);
      if (!Number.isFinite(value) || value < 0) throw new Error('Uno de los objetivos guardados no es válido.');
      goals[key] = value;
    });
    return Object.assign({}, emptyState, saved, { goals: goals, activeWorkout: saved.activeWorkout || null });
  }
  function importBackupFile(file) {
    var errorNode = document.getElementById('backup-error');
    if (!file) return;
    var reader = new FileReader();
    reader.onerror = function () { if (errorNode) errorNode.textContent = 'No se pudo leer el archivo.'; };
    reader.onload = function () {
      try {
        var restored = validateBackup(JSON.parse(String(reader.result || '')));
        if (!window.confirm('Restaurar esta copia reemplazará los datos de FitAI guardados en este navegador. ¿Quieres continuar?')) return;
        localStorage.setItem(STORE_KEY, JSON.stringify(restored));
        state = restored; closeModal(); renderAll(); toast('Copia restaurada en este dispositivo.');
      } catch (error) {
        if (errorNode) errorNode.textContent = error.message || 'No se pudo restaurar la copia.';
      }
    };
  ...
  function openFreeAssistant(question) {
    ...
    document.addEventListener('change', function (event) {
      if (event.target.id === 'backup-file') {
        importBackupFile(event.target.files && event.target.files[0]); return;
      }
